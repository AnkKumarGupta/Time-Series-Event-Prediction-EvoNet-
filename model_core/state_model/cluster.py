...
class ClusterStateRecognition(BaseMLModelTemplate):
    ...
    def restore(self, path, **kwargs):
        # # Use provided path or fall back to param
        # base_path = path or self.param.model_save_path
        # save_name = f"gmm_{self.param.data_name}_{self.param.n_state}.state_model"
        # full_path = os.path.join(base_path, save_name)
        # if not os.path.exists(full_path):
        #     raise FileNotFoundError(f"State model file not found: {full_path}")
