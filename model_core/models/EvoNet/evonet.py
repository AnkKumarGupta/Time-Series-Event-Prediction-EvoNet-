...
class EvoNet():
    ...
    def TemporalModeling(self, event, middle_node_emb, prev_graph_emb, prev_graph_mem, prev_node_emb, prev_node_mem):
        ...
        cur_node_emb, cur_node_mem = self.__GRUUnit__(h_input, prev_node_emb, name='node_gru')
        ...
        cur_graph_emb, cur_graph_mem  = self.__GRUUnit__(g_input, prev_graph_emb, name='graph_gru')
    ...
    def __GRUUnit__(self, input_x, prev_hidden, name):
        with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
            ...
            if name == 'node_gru':
                ...
                W_z = self.__weights__(self.n_features + self.graph_dim, self.n_features, f"Wz_n", init=True)
                U_z = self.__weights__(self.n_features, self.n_features, f"Uz_n", init=True)
                b_z = self.__bias__(self.n_features, f"bz_n", init=True)

                W_r = self.__weights__(self.n_features + self.graph_dim, self.n_features, f"Wr_n", init=True)
                U_r = self.__weights__(self.n_features, self.n_features, f"Ur_n", init=True)
                b_r = self.__bias__(self.n_features, f"br_n", init=True)

                W_h = self.__weights__(self.n_features + self.graph_dim, self.n_features, f"Wh_n", init=True)
                U_h = self.__weights__(self.n_features, self.n_features, f"Uh_n", init=True)
                b_h = self.__bias__(self.n_features, f"bh_n", init=True)

                z = tf.sigmoid(tf.matmul(input_x, W_z) + tf.matmul(prev_hidden, U_z) + b_z)
                r = tf.sigmoid(tf.matmul(input_x, W_r) + tf.matmul(prev_hidden, U_r) + b_r)
                h_ = tf.tanh(tf.matmul(input_x, W_h) + tf.matmul(r * prev_hidden, U_h) + b_h)
                h = (1 - z) * prev_hidden + z * h_

                h = tf.reshape(h, [-1, self.n_nodes, self.n_features])
                return h, h  # h is used for both hidden and memory to keep interface consistent

            elif name == 'graph_gru':
                W_z = self.__weights__(self.n_features + self.n_event, self.graph_dim, f"Wz_g", init=True)
                U_z = self.__weights__(self.graph_dim, self.graph_dim, f"Uz_g", init=True)
                b_z = self.__bias__(self.graph_dim, f"bz_g", init=True)

                W_r = self.__weights__(self.n_features + self.n_event, self.graph_dim, f"Wr_g", init=True)
                U_r = self.__weights__(self.graph_dim, self.graph_dim, f"Ur_g", init=True)
                b_r = self.__bias__(self.graph_dim, f"br_g", init=True)

                W_h = self.__weights__(self.n_features + self.n_event, self.graph_dim, f"Wh_g", init=True)
                U_h = self.__weights__(self.graph_dim, self.graph_dim, f"Uh_g", init=True)
                b_h = self.__bias__(self.graph_dim, f"bh_g", init=True)
                ...
                raise Exception('Unknown GRU unit name')
