...
class EvoNet_TSC(BaseMLModelTemplate):
    ...
    def build_model(self, is_training):
        ...
        dense1 = Dense(512, activation=tf.nn.relu, name="outnet_fc1")
        net = dense1(patterns)
        dense2 = Dense(self.param.n_event, activation=None, name="outnet_fc2")
        out_logits = dense2(net)

        gamma = 0.5   
        alpha = 1.5 

        labels = tf.reshape(y_clf[:, 2:], [-1, self.param.n_event])    
        logits = out_logits                                          
        ...
        alpha_factor = labels[:,1] * alpha + labels[:,0] * 1.0

        focal_losses = alpha_factor * modulator * ce

        self.net_loss = tf.reduce_mean(focal_losses)
